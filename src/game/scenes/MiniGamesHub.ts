...
export class MiniGamesHub extends Phaser.Scene {
  ...
  create(): void {
    ...
    this.add.text(width / 2, 86, strings.miniGames.hubSubtitle, {
      fontFamily: 'VT323, ui-monospace', fontSize: '16px', color: '#8a8d92', align: 'center',
    }).setOrigin(0.5);
    ...
    cards.forEach(([title, desc, icon, onClick], i) => {
      ...
      this.add.text(x, y + 40, desc, {
        fontFamily: 'VT323, ui-monospace', fontSize: '12px', color: '#8a8d92', align: 'center',
        wordWrap: { width: 220 },
      }).setOrigin(0.5);
    ...
    guided.on('pointerup', () => this.scene.start('GuidedSession'));

    const back = this.add.text(40, 40, '← ' + strings.miniGames.back, {
      fontFamily: 'VT323, ui-monospace', fontSize: '16px', color: '#f5f5f5',
